...
\tcbset{
  codebox/.style={
    colback=gray!7, colframe=gray!30, arc=3pt, boxrule=0.4pt,
...
% ---- Example environment -----------------------------------
% Code box occupies 0.54\linewidth; diagram gets the remaining 0.44\linewidth
% (0.02\linewidth gutter between them).
%
% \minipage/\endminipage are called directly (not via \begin/\end) so that
% LaTeX's environment-nesting checker does not object to the minipage being
% opened inside rkexample and closed inside \rksidediagram.
\newenvironment{rkexample}[1]{%
  \medskip\noindent\textit{``#1''}\par\smallskip
  \noindent\minipage[t]{0.54\linewidth}%   <- raw call, no env tracking
    \begin{tcolorbox}[codebox]%
}{%
  \end{tcolorbox}%
}% \minipage intentionally left open — \rksidediagram closes it

% \rksidediagram{content} — closes the raw \minipage from rkexample then
% opens a fresh minipage flush to its right.  adjustbox scales the diagram
% down only when it exceeds 0.44\linewidth; smaller diagrams are untouched.
\newcommand{\rksidediagram}[1]{%
  \endminipage%                            <- closes the raw \minipage
