...
\testcase{She ran into the old barn near the river.}{%
  \begin{rkdiagram}
    \rkSubject{She}
    \rkVerb[%
      \rkPrep[\rkAdj{the}, \rkAdj{old}, \rkPrep[\rkAdj{the}]{near}{river}]{into}{barn}
    ]{ran}
  \end{rkdiagram}
...
\testcase{He painted the wall but not the ceiling.}{%
  \begin{rkdiagram}
    ...
    \rkVerb{painted}
    \rkDObj{%
      \rkCompound[but not][\rkAdj{the}]{wall}[\rkAdj{the}]{ceiling}%
    }
  \end{rkdiagram}
...
\testcase{John and Maria quickly found the lost dog and gently
  ...
  \begin{rkdiagram}
    ...
    \rkVerb{%
      \rkCompound[and]%
        [\rkAdv{quickly},\rkDObj[\rkAdj{the},\rkAdj{lost}]{dog}]{found}%
        [\rkAdv{gently},\rkDObj{it},\rkAdv{home}]{carried}%
    }
  \end{rkdiagram}
...
\begin{tabular}{lp{5.5cm}p{7cm}}
  ...
  28--32 & Go! / Birds fly / NYC / has been running / never &
...
take us ashore at so much a head, silver coin of any country.}{%
  ...
  \begin{rkdiagram}[spacing=1.0cm,modDepth=0.9cm,cmpSep=1.8cm]
    \rkSubject[%
      \rkAdj{A},
      \rkPrep[%
        \rkAdj{swarthy},
