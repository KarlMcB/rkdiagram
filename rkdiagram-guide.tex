...
\begin{tabular}{@{}l l p{8cm}@{}}
  ...
  \opt{gerundPostH}    & \opt{0pt}    & Force exact post height, overriding adaptive calc (0pt = adaptive) \\
  \opt{gerundShelfExt} & \opt{0pt}    & Extra width added to the right of the lower shelf and triangle foot \\
...
The optional \texttt{[mods]} argument holds modifiers of the gerund itself (adjectives, prepositional phrases, etc.). The package pre-scans the list to compute a post height tall enough to keep all modifier slants clear of the foot. Use \opt{gerundPost} (default \texttt{1.2cm}) to set a global minimum post height. Two additional \cmd{rkset} controls allow per-gerund manual adjustment — see the subsections below.
\newpage
...
\subsection*{\opt{gerundPostH} — force exact post height}

Normally the vertical bar height is chosen adaptively: at least \opt{gerundPost}, and taller if the gerund's own modifier slants require it. \opt{gerundPostH} overrides both constraints and sets the post to exactly the given length. Set it back to \texttt{0pt} to restore adaptive behaviour for subsequent gerunds.

\begin{lstlisting}
\rkset{gerundPostH=2.5cm}  % force this one gerund's post to 2.5 cm
\rkSubject{\rkGerund{Swimming}}
\rkset{gerundPostH=0pt}    % restore adaptive mode
\end{lstlisting}

\medskip
\begin{rkexample}{Swimming is fun. (forced post height)}
\begin{lstlisting}
\begin{rkdiagram}
...
\subsection*{\opt{gerundShelfExt} — extend the lower shelf}

By default the lower shelf spans exactly the gerund word's width. When a gerund has wide or numerous modifiers, their slant lines may extend past the shelf's right edge. \opt{gerundShelfExt} adds extra length to the right of the lower shelf and the triangle foot, giving the modifiers more room.

\begin{lstlisting}
\rkset{gerundShelfExt=1.5cm}
\rkSubject{\rkGerund[\rkAdj{Rapid}]{swimming}}
\rkset{gerundShelfExt=0pt}
\end{lstlisting}

\medskip
\begin{rkexample}{Rapid swimming exhausts him. (extended shelf)}
\begin{lstlisting}
\begin{rkdiagram}
  \rkset{gerundShelfExt=1.5cm}
  \rkSubject{
...
\begin{tabular}{@{}p{7cm}p{8cm}@{}}
  ...
  \opt{gerundPost=\textit{len}}      & Global minimum post height for gerund vertical bars \\
  \opt{gerundPostH=\textit{len}}     & Force exact post height for the next gerund (\texttt{0pt} = adaptive) \\
  \opt{gerundShelfExt=\textit{len}}  & Extend gerund lower shelf and foot to the right (\texttt{0pt} = auto) \\
  \bottomrule
\end{tabular}
